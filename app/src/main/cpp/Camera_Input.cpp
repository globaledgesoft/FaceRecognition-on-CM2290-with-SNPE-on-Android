...
Camera_Input::Camera_Input(camera_type type) {
...
bool Camera_Input::MatchCaptureSizeRequest(ImageFormat* resView, int32_t width, int32_t height) {
  ...
  if (foundIt) {
    resView->width = 1920;//foundRes.org_width();
    resView->height = 1080;//foundRes.org_height();
  } else {
    if (disp.IsPortrait()) {
      resView->width = 1080;
      resView->height = 1920;
...
bool Camera_Input::CreateCaptureSession(ANativeWindow* window) {
