...
void OnImageCallback(void *ctx, AImageReader *reader) {
  reinterpret_cast<Input_Image *>(ctx)->ImageCallback(reader);
}


Input_Image::Input_Image(ImageFormat *res, enum AIMAGE_FORMATS format)
    : reader_(nullptr),
      presentRotation_(0),
...
void Input_Image::ImageCallback(AImageReader *reader) {
  int32_t format;
...
ANativeWindow *Input_Image::GetNativeWindow(void) {
...
AImage *Input_Image::GetNextImage(void) {
  AImage *image;
...
/**
 *   Retrieve the Last image in Input_Image's bufferQueue, images may be
 * skipped
 */
AImage *Input_Image::GetLatestImage(void) {
  AImage *image;
 ...
 */
int32_t Input_Image::GetMaxImage(void) {
  int32_t image_count;
 ...
 */
void Input_Image::DeleteImage(AImage *image) {
  if (image != nullptr) {
    AImage_delete(image);
    image = nullptr;
  }
...
bool Input_Image::DisplayImage(ANativeWindow_Buffer *buf, AImage *image) {
 ...
 */
void Input_Image::PresentImage(ANativeWindow_Buffer *buf, AImage *image) {
 ...
 */
void Input_Image::PresentImage90(ANativeWindow_Buffer *buf, AImage *image) {
  AImageCropRect srcRect;
  AImage_getCropRect(image, &srcRect);
...
void Input_Image::PresentImage180(ANativeWindow_Buffer *buf, AImage *image) {
  AImageCropRect srcRect;
  AImage_getCropRect(image, &srcRect);
 ...
 */
void Input_Image::PresentImage270(ANativeWindow_Buffer *buf, AImage *image) {
  AImageCropRect srcRect;
  AImage_getCropRect(image, &srcRect);
...
void Input_Image::SetPresentRotation(int32_t angle) {
