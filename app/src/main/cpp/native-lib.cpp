...
Java_com_detect_facerecognition_MainActivity_onCreateJNI(JNIEnv *env, jobject clazz, jobject activity, jobject j_asset_manager) {
    LOGI("+++++++++++++++++++++++++++++++++++++++");
...
Java_com_detect_facerecognition_MainActivity_setSurface(
        JNIEnv* env, jobject clazz,
        jobject surface) {

    app.SetNativeWindow(ANativeWindow_fromSurface(env, surface));
    app.SetUpCamera();
    std::thread loopThread(&FaceRecApp::CameraLoop, &app);
    loopThread.detach();
}
